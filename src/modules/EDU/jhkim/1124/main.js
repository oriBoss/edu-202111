...
function onButtonClick2(/* cpr.events.CMouseEvent */ e){
	...
	if (checkRowIndices.length > 0){
		...
		if (confirmResult){
			...
			subSetRequestEncoder(subDelete, "ds1Copy");//미라콤에서 데이터 보내주는 형식으로 인코딩처리해온데이터
			subDelete.send();
		}
	}
	else {
...
function onButtonClick3(/* cpr.events.CMouseEvent */ e){
	...
	app.openDialog("app/dialogPopup", {width : 900, height : 600}, function(dialog){
...
function onGrd1CellClick(/* cpr.events.CGridMouseEvent */ e){
	...
	rowState = grd1.getRowState(e.rowIndex);
