...
/*
 * "Delete" 버튼(btn1)에서 click 이벤트 발생 시 호출.
 * 사용자가 컨트롤을 클릭할 때 발생하는 이벤트.
 */
function onBtn1Click(/* cpr.events.CMouseEvent */ e){
	...
	if (checkRowIndices.length > 0){
		...
		if (confirmResult){
			var ds1 = app.lookup("dsFuctionList"); // 그리드에 바인딩된 데이터 셋
			var ds1Copy = app.lookup("dsFuctionCopy"); // 그리드에서 check한 행만 따로 담아둘 데이터 셋, 미라콤 프로젝트의 규약에 맞게 데이터를 넘겨주기 위해 check한 행만 따로 담아둠
...
/*
 * "New" 버튼(btn3)에서 click 이벤트 발생 시 호출.
 * 사용자가 컨트롤을 클릭할 때 발생하는 이벤트.
 */
